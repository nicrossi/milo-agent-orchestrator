...
**The student** — passes the assignment, fails the exam, doesn't know why. AI gave them fluency without competence. They lose the metacognitive habit of *noticing what they don't know*.

**The teacher** — used to read a wrong answer and infer the misconception behind it. Now every answer is technically correct, so misconceptions go undetected until the next high-stakes assessment.

**The institution** — accreditation depends on certifying competence. If grades no longer map to cognition, grades stop being a credential.
...
It refuses to give answers, follows a pedagogical state machine, and turns each conversation into a structured record of how the student thought.
...
**Why this matters**
Milo's questioning is **goal-directed**, not generic. It knows what the student is supposed to learn.
...
**What Milo does**
Sends an unsolicited opening question grounded in the activity goal — not *"how can I help?"* but *"how would you describe this problem in your own words?"*

**What is captured**
The session begins in the **PLANNING** state of a finite state machine.

**Why this matters**
The student is forced to externalize their starting point **before any AI assistance is possible**.
...
**What Milo does**
Extracts five signals from the text: *attempt_present · hedging · confusion · miscalibration · affect_load*.

A finite state machine then chooses a question family — goal clarification, self-explanation, calibration probe, discrepancy detection, transfer probe, or **stabilize** if overwhelmed.

**What is captured**
A structured signature of this turn — what reasoning markers appeared, what state the student is in.

**Why this matters**
Milo is not free-styling. Every question belongs to a pedagogical family **chosen by rules, not vibes**.
...
**What Milo does**
Scores every sentence of the LLM's reply for **open-endedness**. If no sentence is sufficiently open — i.e., the LLM tried to give an answer — Milo **rewrites the response** by appending the planned Socratic question and logs a `direct_answer_leakage` event.

**What is captured**
A guarantee that what reaches the student contains a real question, plus a metric showing how often the LLM tried to slip.

**Why this matters**
"No answers" is **not a prompt instruction** — it is a deterministic output filter. The system enforces the policy structurally.
...
**What Milo does**
Closes the session and runs a second LLM pass over the **full transcript** against a fixed rubric.
...
**Why this matters**
A free-text conversation becomes a **structured row in a database**. Conversations become comparable across students, classes, and cohorts.
...
**What Milo does**
Plots every student on a **calibration quadrant** — *well-calibrated · overconfident · underconfident · struggling* — with the most at-risk students sorted to the top, each annotated with reflection-quality flags and a recommended action.
...
**The core argument:** Milo replaces a broken artifact (the answer) with a new artifact (the scored reasoning trace).
...
A first-year CS student. Activity: *Why does binary search require a sorted input?*
...
**Cannot explain it on the exam two weeks later.** Teacher learns this in week 12.
...
The dialogue you just saw was **not improvised**. It was the output of a deterministic policy.
...
Not a prompt instruction — a deterministic output filter that rewrites any answer into a question.

**It produces structured cognitive evidence.**
Every session ends as a row with reflection quality, calibration, and transfer — each cited with the student's own words.

**It restores the teacher's diagnostic surface.**
A class-level dashboard ranked by intervention urgency, derived from how students reasoned, not from what they submitted.
